...
#define WINDOW_TITLE "03 Background + Character"
...
int main(void) {
    ...
    while (running) {
        ...
        // this shit draw background
        SDL_RenderCopy(game.renderer, game.background, NULL, NULL);

        // this shit draw character (in the midle of nowhere)
        int char_width = 0, char_height = 0;
        SDL_QueryTexture(game.character, NULL, NULL, &char_width, &char_height);

        SDL_Rect dest = {
            (SCREEN_WIDTH - char_width) / 2,   // X 
            (SCREEN_HEIGHT - char_height) / 2, // Y 
            char_width,
            char_height
        };
