...
int main(void) {
    ...
    if (timeFile != NULL) {
        ...
        if (fgets(buffer, sizeof(buffer), timeFile) != NULL) {
            sscanf(buffer, "w%dr%d", &workMinutes, &restMinutes);
        }
        fclose(timeFile);
    }   
    ...
    int totalWorkTime = workMinutes * 60; 
    int totalRestTime = restMinutes * 60; 
    SDL_Event e;
    int selectedTotalTime = totalWorkTime;
    bool isResting = false;
    bool timerFinished = false;
    
    Uint32 lastHungerTickTime = SDL_GetTicks();
    
    printf("Work! Time: %d min\n", workMinutes);
    ...
    while (running) {
        ...
        if (!timerFinished) {
            ...
            if (isResting){
                SetCursorPos(SCREEN_WIDTH, SCREEN_HEIGHT   );
            }
        ...
        SDL_Color timerColor = isResting ? (SDL_Color){19, 24, 16,255} : (SDL_Color){19, 24, 16,255};
        char timeText[32];
        snprintf(timeText, sizeof(timeText), "%02d:%02d", remaining / 60, remaining % 60);
        if (game_global.fontTimer) {
            renderTextCentered(game_global.renderer, game_global.fontTimer, timeText, 71, SCREEN_HEIGHT - 21, timerColor);
        }
